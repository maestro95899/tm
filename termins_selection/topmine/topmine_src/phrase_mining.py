...
class PhraseMining(object):
    ...
    def _frequentPatternMining(self, documents, min_support, max_phrase_size, word_freq, active_indices):
        ...
        """
        print("Start _frequentPatternMining")
        print("_frequentPatternMining: len(documents) =", len(documents))
        hash_counter = word_freq
        n = 2
        
        #iterate until documents is empty
        while(len(documents) > 0):
            print("circle", n, "len(documents) =", len(documents))
            temp_documents = []
            new_active_indices = []
                        ...
                        if (new_word_indices[idx+1] == i + 1):
                            for idx in range(i, i+n):
                                if idx == i+n-1:
                                    ...
                                    phrase += words[idx] + " "
                        # модификацию про документы вставлять сюда
                        hash_counter[phrase] += 1
                        #tmp_hash_counter[phrase] += 1
                    for key, value in tmp_hash_counter.items():
                        #hash_counter[key] += 1
                        pass
            ...
            n += 1
            print("Process _frequentPatternMining: n =", n)
        ...
        ##
        pi = 0
        for pkey, pval in hash_counter.items():
            print(pi)
            print(pkey, pval)
            print()
            pi += 1
    ...
    def _preprocess_input(self, filename, stopwords):
        ...
        for line in f:
            ...
            sentences_no_punc = [" ".join([sentence for sentence in sentences_no_punc])]
            stripped_sentences = []
            for sentence in sentences_no_punc:
                stripped_sentences.append(re.sub('[^A-Za-z0-9А-Яа-я]+', ' ', sentence))
    ...
    def _run_phrase_mining(self, min_support, max_phrase_size, alpha, file_name):
        ...
        print("len(doc_phrases) =", len(doc_phrases))
    ...
    def get_frequent_phrases(self, min_support, counter_type="true"):
        """
        Returns the most frequent phrases in the corpus that occur more than 
        the minimum support in descending order of frequency
        """
        frequent_phrases = []
        if counter_type == "true":
            counter = self.true_counter
        elif counter_type == "hash":
            counter = self.hash_counter
